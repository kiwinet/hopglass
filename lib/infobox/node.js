define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showStatus(d) {
    return function (el) {
      ...
      if (d.flags.online)
        el.textContent = "ijungtas, paskutinis kartas " + d.lastseen.fromNow() + " (" + d.lastseen.format("YYYY.MM.DD,  H:mm:ss") + ")"
      else
        el.textContent = "išjungtas, paskutinis kartas " + d.lastseen.fromNow() + " (" + d.lastseen.format("YYYY.MM.DD,  H:mm:ss") + ")"
    }
  }
  ...
  function wifiChannelAlias(ch) {
    var chlist = {
      ...
      "136": "5680 MHz (DFS) !!",
      "140": "5700 MHz (DFS) !!",
      "149": "5745 MHz (SRD) 25 mW !!",
      "151": "5755 MHz (SRD) 25 mW !!",
      "153": "5765 MHz (SRD) 25 mW !!",
      "155": "5775 MHz (SRD) 25 mW !!",
      "157": "5785 MHz (SRD) 25 mW !!",
      "159": "5795 MHz (SRD) 25 mW !!",
      "161": "5805 MHz (SRD) 25 mW !!",
      "165": "5825 MHz (SRD) 25 mW !!"
    }
    if (!(ch in chlist))
      return ""
    else
  ...
  function showClients(d) {
    ...
    return function (el) {
      el.appendChild(document.createTextNode(d.statistics.clients > 0 ? d.statistics.clients : "nėra"))
  ...
  function showAutoupdate(d) {
    ...
    return au.enabled ? "įjungtas (" + au.branch + ")" : "išjungtas"
  }

  function showStatImg(o, d) {
    var subst = {}
    subst["{NODE_ID}"] = d.nodeinfo.node_id ? d.nodeinfo.node_id : "nežinomas"
    subst["{NODE_NAME}"] = d.nodeinfo.hostname ? d.nodeinfo.hostname : "nežinomas"
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Statusas", showStatus(d))
    attributeEntry(attributes, "Gateway", d.flags.gateway ? "ja" : null)
    attributeEntry(attributes, "Koordinatės", showGeoURI(d))

    if (config.showContact)
      attributeEntry(attributes, "Kontaktai", dictGet(d.nodeinfo, ["owner", "contact"]))

    attributeEntry(attributes, "Įranga",  dictGet(d.nodeinfo, ["hardware", "model"]))
    attributeEntry(attributes, "Pirminis MAC", dictGet(d.nodeinfo, ["network", "mac"]))
    attributeEntry(attributes, "Mazgo ID", dictGet(d.nodeinfo, ["node_id"]))
    attributeEntry(attributes, "Firmware", showFirmware(d))
    attributeEntry(attributes, "Vieta", showSite(d, config))
    attributeEntry(attributes, "Uptime", showUptime(d))
    attributeEntry(attributes, "Tinklo dalis", showFirstseen(d))
    attributeEntry(attributes, "Kanalas 2.4 GHz", showWifiChannel(dictGet(d.nodeinfo, ["wireless", "chan2"])))
    attributeEntry(attributes, "Kanalas 5 GHz", showWifiChannel(dictGet(d.nodeinfo, ["wireless", "chan5"])))
    attributeEntry(attributes, "Eterio laikas 2.4 GHz", showAirtime(2, dictGet(d.statistics, ["wireless", "airtime2"])))
    attributeEntry(attributes, "Eterio laikas 5 GHz", showAirtime(5, dictGet(d.statistics, ["wireless", "airtime5"])))
    attributeEntry(attributes, "Apkrova", showLoad(d))
    attributeEntry(attributes, "Atmintis (Ram)", showRAM(d))
    attributeEntry(attributes, "IP Adresai", showIPs(d))
    attributeEntry(attributes, "Tinklapis", showPages(d))
    attributeEntry(attributes, "Pasirinktas Gateway", showGateway(d, router))
    attributeEntry(attributes, "Autom. Atnaujinimas", showAutoupdate(d))
    attributeEntry(attributes, "Klientai", showClients(d))
    ...
    if (d.neighbours.length > 0) {
      ...
      h3.textContent = "Jungtys (" + d.neighbours.length + ")"
      el.appendChild(h3)
      ...
      th4.textContent = "Tipas"
      tr.appendChild(th4)

      var th5 = document.createElement("th")
      th5.textContent = "Atstumas"
      tr.appendChild(th5)
